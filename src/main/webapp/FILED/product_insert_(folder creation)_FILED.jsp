...
<body>
	...
	for (int i = 0; i < productDao.select_productImage_dao().size(); i++) {
		...
		if (productDao.select_productImage_dao().get(i).getIdProduct() == productDao.getResult_listProduct_Dao().get(0)
		.getIdProductRegistration()) {
			resultReturn = true;
		}

	}

	if (productDao.getResult_productImageList_Dao().isEmpty() && resultReturn == false) {

		productImage_obj.setIdProduct(productDao.getResult_listProduct_Dao().get(0).getIdProductRegistration());
