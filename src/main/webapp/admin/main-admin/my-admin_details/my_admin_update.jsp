...
<html>
<head>
<%@ page language="java" errorPage="../error-redirection_admin-update.jsp"%>
...
<body>
	...
	if(request.getParameter("fnameAdmin_update").isBlank()){
		 adminRegistrationDao.returnAccess_Administrator_dao().get(0).getIdAdmin(); 
	}else{
	...
	if(request.getParameter("femailAccess_update").isBlank()){
		 adminRegistrationDao.returnAccess_Administrator_dao().get(0).getEmailAccess(); 
	}else{
	...
	if(request.getParameter("faccessLevel_update").isBlank()){
		 adminRegistrationDao.returnAccess_Administrator_dao().get(0).getAccessLevel(); 
	}else{
	...
	if(request.getParameter("fadminPassword_update").isBlank()){
		 adminRegistrationDao.returnAccess_Administrator_dao().get(0).getAdminPassword(); 
	}else{
	...
	if(request.getParameter("fphoneContact_update").isBlank()){
		 adminRegistrationDao.returnAccess_Administrator_dao().get(0).getPhoneContact(); 
	}else{
